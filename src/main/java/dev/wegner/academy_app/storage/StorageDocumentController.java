...
public class StorageDocumentController {
    ...
    public List<StorageDocumentResponse> findDocuments(@PathVariable Long studentId) {
        ...
        return storageDocumentRepository.findByStudentId(studentId).stream().map(this::toResponse).toList();
    }

    @GetMapping("/documents/{documentId}/download")
    public ResponseEntity<InputStreamResource> download(@PathVariable Long documentId) throws Exception {
        var document = storageDocumentRepository.findById(documentId).orElseThrow();
        var stream = storageDocumentService.download(documentId);

        return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + document.getFileName() + "\"").contentType(MediaType.parseMediaType(document.getContentType())).body(new InputStreamResource(stream));
    }
